...
def make_dataframe(links):                   #returns the dataframe containing columns of name,city,location,# of doctors, # of beds
    ...
    df2, df3,df4,df5, df_links = pd.DataFrame([]), pd.DataFrame([]),pd.DataFrame([]),pd.DataFrame([]),pd.DataFrame([])
    ...
    for city,city_name in zip(cities_links,cities):
            ...
            for city_page in city:
                ...
                df_link_temp = pd.DataFrame(hosp_urls)
                df_links = df_links.append(df_link_temp)
    ...
    df_links.reset_index(list(range(pages_per_city*hosp_per_page*len(cities))),drop=True, inplace=True)
...
# df.reset_index(list(range(len(df))),drop=True, inplace=True)
...
#CODE BROKEN (no.of beds doesn't match for two or more cities)
#Work in progress